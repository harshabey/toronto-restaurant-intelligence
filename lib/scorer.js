...
function rankRestaurants(restaurants, params) {
  ...
  const scored = restaurants.map(r => {
    ...
    // ── Location (30 pts exact, up to 15 pts distance-based) ──────────────
    if (location) {
    ...
    // ── Occasion (25 pts + special rules) ────────────────────────────────
    const occasionMatch = r.occasions.includes(occasion);
    if (occasionMatch) {
      score += 25;
      matchFactors.occasionMatch = true;
    }

    // Hard-filter occasions: venue must be eligible type
